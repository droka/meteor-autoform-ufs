...
import { imageIsHostedByUs, src2image } from './template-helper.js';
...
Template.afFileUpload.helpers({
  ...
  textOrHidden() {
    ...
    if (!vmValue) return "text";
    return imageIsHostedByUs(vmValue) ? "hidden" : "text";
  },
  valueHasChanged() {
  ...
  'change [data-files-collection-upload]'(e, template) {
    ...
    const uploader = new UploadFS.Uploader({
      ...
      onComplete(file) {
        ...
        template.currentUpload.set(null);
        template.viewmodelValue.set(file.path);
      },
      onCreate(file) {
        console.log(file.name + ' has been created with ID ' + file._id);
        template.viewmodelValue.set('/images/avatars/avatarnull.png');
      },
